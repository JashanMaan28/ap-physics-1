...
function generateProblem(): Problem {
  ...
  switch (type) {
    ...
    case 2: {
      ...
      const v = 9.8 * t;
      return { category: "Free Fall", question: `An object is dropped from ${h} m. How long does it take to hit the ground? (g = 9.8 m/s²)`, answer: `${t.toFixed(2)} s`, solution: `h = ½gt² → t = √(2h/g) = √(2×${h}/9.8) = ${t.toFixed(2)} s. Final velocity = gt = ${v.toFixed(1)} m/s.` };
    }
    default: {
  ...
  return (
    <Card className="max-w-2xl mx-auto">
      ...
      <CardContent className="space-y-4">
        ...
        <p className="text-base font-medium">{problem.question}</p>
        <Separator />
        {showSolution ? (
          <div className="space-y-2 rounded-lg bg-muted/50 border p-4">
            <p className="text-sm font-semibold">Answer: {problem.answer}</p>
            <p className="text-sm text-muted-foreground whitespace-pre-line">{problem.solution}</p>
          </div>
        ) : (
